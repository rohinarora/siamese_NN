...
def faceRecoModel(input_shape):
    ...
    X_input = Input(input_shape)  # input shape is (Nc, Nh, Nw). (m,Nc, Nh, Nw) is implicit?

    # Zero-Padding
    X = ZeroPadding2D((3, 3),data_format='channels_first')(X_input) 
    # in each function we explicitly mention data_format='channels_first'
    # this is redundant as we already say keras.backend.set_image_data_format('channels_first')

    
    # First Block
    X = Conv2D(64, (7, 7), strides = (2, 2), name = 'conv1',data_format='channels_first')(X)
    X = BatchNormalization(axis = 1, name = 'bn1')(X)  # axis =1 because channel first convention here
    X = Activation('relu')(X)
